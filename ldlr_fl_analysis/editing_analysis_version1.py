...
sequence_columns = ["Spacer_fw", "7-nt_barcode_rc_orientation", "36nt_Unedited", "36nt_Edited"]
df[sequence_columns] = df[sequence_columns].apply(lambda x: x.str.upper())

# Create lookup dictionaries
barcode_to_spacer = dict(zip(df["7-nt_barcode_rc_orientation"], df["Spacer_fw"]))
barcode_to_pegRNA = dict(zip(df["7-nt_barcode_rc_orientation"], df["pegRNA_name"]))
barcode_to_unedited = dict(zip(df["7-nt_barcode_rc_orientation"], df["36nt_Unedited"]))
barcode_to_edited = dict(zip(df["7-nt_barcode_rc_orientation"], df["36nt_Edited"]))
...
def process_fastq(r1_file, r2_file):
    global total_unedited, total_edited, total_neither
